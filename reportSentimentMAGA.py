...
def visualization():
    ...
    query="select prediction, sum(total_label) as suma from sentiment1 where \
            date between cast('{}' as timestamp)- INTERVAL 1 HOUR and cast('{}' as timestamp) and label='MAGA' \
    ...
    plt.axis('equal')
    plt.suptitle("Sentiment MAGA", fontsize = 20)
    plt.tight_layout()
    plt.show()      
...
if __name__ == "__main__":
    sc = SparkContext(appName="Sentiment MAGA")
    visualization()
